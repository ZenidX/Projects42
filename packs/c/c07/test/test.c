...
static void	t_range(int min, int max)
{
	...
	if (min >= max)
	{
		if (!res(r == NULL, "ft_range(%d, %d) -> NULL", min, max))
	...
	if (!res(ok, "ft_range(%d, %d) -> %d elementos", min, max, max - min))
	{
		if (!r)
			detail("devuelve NULL");
		else
...
static void	t_ultimate_range(int min, int max)
{
	...
	{
		if (!res(n == 0 && r == NULL,
				"ft_ultimate_range(&r, %d, %d) -> 0 y r = NULL", min, max))
	...
	if (!res(ok, "ft_ultimate_range(&r, %d, %d) -> %d", min, max, max - min))
	{
		if (n != max - min)
