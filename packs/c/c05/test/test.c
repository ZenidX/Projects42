...
static void	t_next_prime(int nb)
{
	...
	r = ref_next_prime(nb);
	if (!res(m == r, "ft_find_next_prime(%d)", nb))
		detail("esperado %d, obtenido %d", r, m);
}
...
static void	run_cases(void)
{
	...
	cap_end();
	if (!res(ret == 724, "ft_ten_queens_puzzle() devuelve 724"))
		detail("esperado 724, obtenido %d", ret);
	len = strlen(g_got);
	if (!res(len == 724 * 11, "la salida son 724 lineas de 10 digitos + \\n"))
	...
	while (lines-- > 0)
		if (g_got[lines * 11 + 10] != '\n'
			...
			bad++;
	if (!res(bad == 0, "todas las lineas son soluciones validas"))
		detail("%d lineas mal formadas o con damas que se atacan", bad);
	lines = (int)(len / 11);
	if (lines > 1000)
		lines = 1000;
	bad = count_dups(g_got, lines);
	if (!res(bad == 0, "no hay soluciones repetidas (primeras %d lineas)",
			lines))
...
{0, "ex00", "ft_iterative_factorial.c"},
{1, "ex01", "ft_recursive_factorial.c"},
{2, "ex02", "ft_iterative_power.c"},
{3, "ex03", "ft_recursive_power.c"},
{4, "ex04", "ft_fibonacci.c"},
{5, "ex05", "ft_sqrt.c"},
{6, "ex06", "ft_is_prime.c"},
{7, "ex07", "ft_find_next_prime.c"},
{8, "ex08", "ft_ten_queens_puzzle.c"},
};
