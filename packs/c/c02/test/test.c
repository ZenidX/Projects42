		...
		const char *name)
{
	...
	while (i < N_STRS)
	{
		...
		r = ref(g_strs[i]);
		if (!res(m == r, "%s(%s)", name, q(g_strs[i])))
		...
		const char *s, const char *name)
{
	...
	ret = mine(m);
	ref(r);
	if (!res(ret == m && memcmp(m, r, sizeof(m)) == 0, "%s(%s)", name, q(s)))
	{
		if (ret != m)
...
static void	t_non_printable(const char *s)
{
	...
	cap_end();
	cmp_out(exp, "ft_putstr_non_printable(%s)", q(s));
}
...
{0, "ex00", "ft_strcpy.c"},
{1, "ex01", "ft_strncpy.c"},
{2, "ex02", "ft_str_is_alpha.c"},
{3, "ex03", "ft_str_is_numeric.c"},
{4, "ex04", "ft_str_is_lowercase.c"},
{5, "ex05", "ft_str_is_uppercase.c"},
{6, "ex06", "ft_str_is_printable.c"},
{7, "ex07", "ft_strupcase.c"},
{8, "ex08", "ft_strlowcase.c"},
{9, "ex09", "ft_strcapitalize.c"},
{10, "ex10", "ft_strlcpy.c"},
{11, "ex11", "ft_putstr_non_printable.c"},
};
