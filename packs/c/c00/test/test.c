...
static void	t_putchar(char c)
{
	...
	ft_putchar(c);
	cap_end();
	cmp_out(exp, "ft_putchar(%s)", q(exp));
}
...
static void	run_cases(void)
{
	...
	ft_print_alphabet();
	cap_end();
	cmp_out("abcdefghijklmnopqrstuvwxyz", "ft_print_alphabet()");
}
...
static void	run_cases(void)
{
	...
	cap_end();
	cmp_out("zyxwvutsrqponmlkjihgfedcba", "ft_print_reverse_alphabet()");
}
...
static void	run_cases(void)
{
	...
	ft_print_numbers();
	cap_end();
	cmp_out("0123456789", "ft_print_numbers()");
}
...
static void	t_is_negative(int n)
{
	...
	ft_is_negative(n);
	cap_end();
	cmp_out(n < 0 ? "N" : "P", "ft_is_negative(%d)", n);
}
...
static char	g_exp[65536];

/* 012, 013, ..., 789   (tres digitos distintos en orden ascendente) */
static void	gen_comb(char *out)
{
	int	a;
	int	b;
	int	c;
...
/* 00 01, 00 02, ..., 98 99 */
static void	gen_comb2(char *out)
{
	int	a;
	int	b;
	int	k;
...
static int	rec_combn(char *out, int k, int *tab, int pos, int n)
{
	int	d;
	int	i;
	int	last;
	...
	d = (pos == 0) ? 0 : tab[pos - 1] + 1;
	while (d <= 9)
	{
		tab[pos] = d;
		k = rec_combn(out, k, tab, pos + 1, n);
		d++;
	}
	return (k);
}

static void	gen_combn(int n, char *out)
{
	int	tab[10];

	out[rec_combn(out, 0, tab, 0, n)] = '\0';
}

static void	t_print_combn(int n)
{
	gen_combn(n, g_exp);
	cap_start();
	ft_print_combn(n);
	cap_end();
	cmp_out(g_exp, "ft_print_combn(%d)", n);
}
...
{0, "ex00", "ft_putchar.c"},
{1, "ex01", "ft_print_alphabet.c"},
{2, "ex02", "ft_print_reverse_alphabet.c"},
{3, "ex03", "ft_print_numbers.c"},
{4, "ex04", "ft_is_negative.c"},
{5, "ex05", "ft_print_comb.c"},
{6, "ex06", "ft_print_comb2.c"},
{7, "ex07", "ft_putnbr.c"},
{8, "ex08", "ft_print_combn.c"},
};
