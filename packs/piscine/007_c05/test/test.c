...
/* nb < 0 no es factorizable -> 0. 0! = 1. No se gestiona el desbordamiento, */
/* por eso los casos llegan solo hasta 12! (el ultimo que cabe en un int).   */
static int	ref_factorial(int nb)
{
	int	r;
	int	i;
...
static void	t_fact(int (*mine)(int), int nb, const char *name)
{
	int	m;
	int	r;

	r = ref_factorial(nb);
	CASE("%s(%d) -> esperado %d", name, nb, r);
	m = mine(nb);
	if (!res(m == r))
		detail("obtenido %d", m);
}
...
static void	run_cases(void)
{
	...
	t_fact(ft_iterative_factorial, -5, "ft_iterative_factorial");
	t_fact(ft_iterative_factorial, -1, "ft_iterative_factorial");
	i = 0;
	while (i <= 12)
	{
		t_fact(ft_iterative_factorial, i, "ft_iterative_factorial");
		i++;
	}
}
...
static void	run_cases(void)
{
	...
	t_fact(ft_recursive_factorial, -5, "ft_recursive_factorial");
	t_fact(ft_recursive_factorial, -1, "ft_recursive_factorial");
	i = 0;
	while (i <= 12)
	{
		t_fact(ft_recursive_factorial, i, "ft_recursive_factorial");
		i++;
	}
}
...
/* power < 0 -> 0. Cualquier numero elevado a 0 vale 1 (tambien 0^0). */
static int	ref_power(int nb, int power)
{
	int	r;
...
static void	t_pow(int (*mine)(int, int), int nb, int p, const char *name)
{
	int	m;
	int	r;

	r = ref_power(nb, p);
	CASE("%s(%d, %d) -> esperado %d", name, nb, p, r);
	m = mine(nb, p);
	if (!res(m == r))
		detail("obtenido %d", m);
}

static void	run_pow(int (*mine)(int, int), const char *name)
{
	int	nb;
	int	p;
	...
	while (nb <= 5)
	{
		...
		while (p <= 5)
		{
			t_pow(mine, nb, p, name);
			p++;
		}
		nb++;
	}
	t_pow(mine, 2, 10, name);
	t_pow(mine, 1, 30, name);
}
...
static void	run_cases(void)
{
	run_pow(ft_iterative_power, "ft_iterative_power");
}
...
static void	run_cases(void)
{
	run_pow(ft_recursive_power, "ft_recursive_power");
}
...
/* index < 0 -> -1. fib(0) = 0, fib(1) = 1. */
static int	ref_fib(int index)
{
	int	a;
	int	b;
	int	t;
...
static void	t_fib(int i)
{
	int	m;
	int	r;

	r = ref_fib(i);
	CASE("ft_fibonacci(%d) -> esperado %d", i, r);
	m = ft_fibonacci(i);
	if (!res(m == r))
		detail("obtenido %d", m);
}
...
/* Devuelve la raiz si nb es un cuadrado perfecto, 0 si no lo es. */
static int	ref_sqrt(int nb)
{
	long	i;
...
static void	t_sqrt(int nb)
{
	int	m;
	int	r;

	r = ref_sqrt(nb);
	CASE("ft_sqrt(%d) -> esperado %d", nb, r);
	m = ft_sqrt(nb);
	if (!res(m == r))
		detail("obtenido %d", m);
}
...
/* 0 y 1 no son primos; los negativos tampoco. */
static int	ref_is_prime(int nb)
{
	long	i;
...
static void	t_is_prime(int nb)
{
	int	m;
	int	r;

	r = ref_is_prime(nb);
	CASE("ft_is_prime(%d) -> esperado %d", nb, r);
	m = ft_is_prime(nb);
	if (!res(m == r))
		detail("obtenido %d", m);
}
...
/* El primer primo mayor o igual que nb. */
static int	ref_next_prime(int nb)
{
	if (nb < 2)
		return (2);
	while (!ref_is_prime(nb))
		nb++;
	return (nb);
}

static void	t_next_prime(int nb)
{
	int	m;
	int	r;

	r = ref_next_prime(nb);
	CASE("ft_find_next_prime(%d) -> esperado %d", nb, r);
	m = ft_find_next_prime(nb);
	if (!res(m == r))
		detail("obtenido %d", m);
}
...
/* Una linea es solucion valida si son 10 digitos sin dos damas en la misma  */
/* fila ni en la misma diagonal (la columna la da la posicion en la linea).  */
static int	valid_solution(const char *l)
{
	int	pos[10];
	int	i;
	int	j;
	...
	while (i < 10)
	{
		...
		while (j < 10)
		{
			if (pos[i] == pos[j] || j - i == pos[j] - pos[i]
				|| j - i == pos[i] - pos[j])
				return (0);
			j++;
		}
...
static int	count_dups(const char *s, int lines)
{
	int	i;
	int	j;
	int	dups;
...
static void	run_cases(void)
{
	...
	CASE("debe devolver 724");
	cap_start();
	ret = ft_ten_queens_puzzle();
	cap_end();
	if (!res(ret == 724))
		detail("esperado 724, obtenido %d", ret);
	len = strlen(g_got);
	case_label("debe imprimir 724 lineas de 10 digitos + \\n");
	if (!res(len == 724 * 11))
		detail("esperados %d bytes, obtenidos %zu", 724 * 11, len);
	lines = (int)(len / 11);
	bad = 0;
	while (lines-- > 0)
		if (g_got[lines * 11 + 10] != '\n'
			|| !valid_solution(g_got + lines * 11))
			bad++;
	case_label("todas las lineas deben ser soluciones validas");
	if (!res(bad == 0))
		detail("%d lineas mal formadas o con damas que se atacan", bad);
	lines = (int)(len / 11);
	if (lines > 1000)
		lines = 1000;
	bad = count_dups(g_got, lines);
	case_label("no debe haber soluciones repetidas (primeras %d lineas)", lines);
	if (!res(bad == 0))
		detail("%d pares de lineas repetidas", bad);
	free(g_got);
	g_got = NULL;
}
...
{0, "ex00", "ft_iterative_factorial.c", NULL},
{1, "ex01", "ft_recursive_factorial.c", NULL},
{2, "ex02", "ft_iterative_power.c", NULL},
{3, "ex03", "ft_recursive_power.c", NULL},
{4, "ex04", "ft_fibonacci.c", NULL},
{5, "ex05", "ft_sqrt.c", NULL},
{6, "ex06", "ft_is_prime.c", NULL},
{7, "ex07", "ft_find_next_prime.c", NULL},
{8, "ex08", "ft_ten_queens_puzzle.c", NULL},
};
