/* ************************************************************************** */
/*                                                                            */
/*   test.c — runner de tests para los ejercicios de C03                      */
/*                                                                            */
...
static void	t_strcmp(char *a, char *b, const char *note)
{
	int	mine;
	int	real;
	...
	mine = ft_strcmp(a, b);
	real = strcmp(a, b);
	if (!res(sgn(mine) == sgn(real)))
	{
		detail("esperado signo %+d, obtenido %+d (devuelve %d)",
			sgn(real), sgn(mine), mine);
		note_line(note);
	}
}
...
static void	run_cases(void)
{
	...
	t_strcmp("a", "", NULL);
	t_strcmp("", "a", NULL);
	t_strcmp("abc", "abc", NULL);
	t_strcmp("abc", "abd", NULL);
	t_strcmp("abd", "abc", NULL);
	t_strcmp("abc", "ab", NULL);
	t_strcmp("ab", "abc", NULL);
	t_strcmp("Saludos", "Salux", NULL);
	t_strcmp("A", "a", NULL);
	t_strcmp("test\200", "test\177",
		"strcmp compara los bytes como unsigned char; con char con signo "
		"0x80 vale -128 y el signo sale al reves");
	t_strcmp("\377", "\1",
		"mismo caso: 0xff debe ser MAYOR que 0x01");
}
...
static void	t_strncmp(char *a, char *b, unsigned int n, const char *note)
{
	int	mine;
	int	real;

	CASE("ft_strncmp(%s, %s, %u)", q(a), q(b), n);
	mine = ft_strncmp(a, b, n);
	real = strncmp(a, b, n);
	if (!res(sgn(mine) == sgn(real)))
	{
		detail("esperado signo %+d, obtenido %+d (devuelve %d)",
...
{
	t_strncmp("", "", 0, NULL);
	t_strncmp("abc", "xyz", 0, "con n == 0 nunca se lee nada, debe dar 0");
	t_strncmp("abc", "abd", 1, NULL);
	t_strncmp("abc", "abd", 2, NULL);
	t_strncmp("abc", "abd", 3, NULL);
	t_strncmp("abc", "abd", 100, NULL);
	t_strncmp("abc", "abc", 3, NULL);
	t_strncmp("abc", "abc", 10, NULL);
	t_strncmp("ab", "abc", 3, NULL);
	t_strncmp("abc", "ab", 3, NULL);
	t_strncmp("", "a", 1, NULL);
	t_strncmp("a", "", 1, NULL);
	t_strncmp("Saludos", "Salux", 5, NULL);
	t_strncmp("test\200", "test\177", 5,
		"strncmp compara los bytes como unsigned char");
}
...
static void	t_strcat(const char *d0, const char *s)
{
	char	mine[256];
	char	real[256];
	char	*ret;
	...
	strcpy(mine, d0);
	strcpy(real, d0);
	CASE("ft_strcat(%s, %s)", q(d0), q(s));
	ret = ft_strcat(mine, (char *)s);
	strcat(real, s);
...
static void	run_cases(void)
{
	...
	t_strcat("a", "b");
	t_strcat("Hola", " mundo");
	t_strcat("Saludos, ", "curpo escombro");
	t_strcat("dest largo con espacios      ", "src");
}
...
static void	t_strncat(const char *d0, const char *s, unsigned int n)
{
	char	mine[256];
	char	real[256];
	char	*ret;
	...
	strcpy(mine, d0);
	strcpy(real, d0);
	CASE("ft_strncat(%s, %s, %u)", q(d0), q(s), n);
	ret = ft_strncat(mine, (char *)s, n);
	strncat(real, s, n);
	if (!res(ret == mine && memcmp(mine, real, sizeof(mine)) == 0))
	{
		if (ret != mine)
...
static void	run_cases(void)
{
	...
	t_strncat("abc", "def", 1);
	t_strncat("abc", "def", 3);
	t_strncat("abc", "def", 100);
	t_strncat("abc", "", 5);
	t_strncat("", "def", 2);
	t_strncat("", "", 0);
	t_strncat("Saludos, ", "curpo escombro", 5);
	t_strncat("Saludos, ", "curpo", 100);
}
...
static void	t_strstr(char *hay, char *nee)
{
	char	*mine;
	char	*real;

	CASE("ft_strstr(%s, %s)", q(hay), q(nee));
	mine = ft_strstr(hay, nee);
	real = strstr(hay, nee);
	if (!res(mine == real))
	{
		detail("esperado %s, obtenido %s", q(real), q(mine));
		if (mine && !real)
			detail("cuando no encuentra nada debe devolver NULL, "
				"no un puntero al final de la cadena");
	}
}

static void	run_cases(void)
{
	t_strstr("Hola mundo mundial", "mundo");
	t_strstr("Hola mundo mundial", "mundial");
	t_strstr("Hola mundo mundial", "Hola");
	t_strstr("Hola mundo mundial", "l");
	t_strstr("Hola mundo mundial", "e");
	t_strstr("Hola mundo mundial", "");
	t_strstr("Hola mundo mundial", "mundoo");
	t_strstr("Hola mundo mundial", "Hola mundo mundial!");
	t_strstr("aaab", "aab");
	t_strstr("aaab", "aaab");
	t_strstr("aaab", "aaaba");
	t_strstr("", "");
...
/* strlcat de BSD, tal cual lo pide el subject: concatena como mucho          */
/* size - 1 bytes y devuelve la longitud que TENDRIA el resultado.           */
static unsigned int	ref_strlcat(char *dst, const char *src, unsigned int size)
{
	unsigned int	dl;
	unsigned int	sl;
	unsigned int	i;
...
static void	t_strlcat(const char *d0, const char *s, unsigned int size)
{
	char			mine[256];
	char			real[256];
	unsigned int	rm;
	...
	strcpy(mine, d0);
	strcpy(real, d0);
	CASE("ft_strlcat(%s, %s, %u)", q(d0), q(s), size);
	rm = ft_strlcat(mine, (char *)s, size);
	rr = ref_strlcat(real, s, size);
	if (!res(rm == rr && memcmp(mine, real, sizeof(mine)) == 0))
	{
		if (rm != rr)
			detail("devuelve %u, esperado %u", rm, rr);
		if (strcmp(mine, real) != 0)
...
static void	run_cases(void)
{
	t_strlcat("Hola", " mundo", 0);
	t_strlcat("Hola", " mundo", 1);
	t_strlcat("Hola", " mundo", 4);
	t_strlcat("Hola", " mundo", 5);
	t_strlcat("Hola", " mundo", 8);
	t_strlcat("Hola", " mundo", 11);
	t_strlcat("Hola", " mundo", 64);
	t_strlcat("", "abc", 10);
	t_strlcat("", "", 10);
	t_strlcat("Hola", "", 10);
	t_strlcat("Saludos, desgraciado!", "buen hombre.", 9);
	t_strlcat("Saludos, desgraciado!", "buen hombre.", 128);
}
...
{0, "ex00", "ft_strcmp.c", NULL},
{1, "ex01", "ft_strncmp.c", NULL},
{2, "ex02", "ft_strcat.c", NULL},
{3, "ex03", "ft_strncat.c", NULL},
{4, "ex04", "ft_strstr.c", NULL},
{5, "ex05", "ft_strlcat.c", NULL},
};
...
int	main(int argc, char **argv)
{
	...
	strcpy(g_tmp, "/tmp/c03test.XXXXXX");
	if (!mkdtemp(g_tmp))
